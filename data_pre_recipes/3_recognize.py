...
def readfile(png_path, dcm_path, des_png_path, des_dcm_path):
    ...
    for roomdir in sorted(os.listdir(png_path)):
        room_png_dir = os.path.join(png_path, roomdir)
        room_dcm_dir = os.path.join(dcm_path, roomdir)
        for patientdir in sorted(os.listdir(room_png_dir)):
            patient_png_dir = os.path.join(room_png_dir, patientdir)
            patient_dcm_dir = os.path.join(room_dcm_dir, patientdir)
            for child in sorted(os.listdir(patient_png_dir)):
                ...
                for dcm_file in sorted(os.listdir(dcm_dir)):
                    ...
                    # print('\nname:', patient, '\ntime:', year + hour, '\nhz:', hz, '\nweek:', week)
                    new_png_file = dcm_file.split('.')[0] + ',' + hz + '.png'
                    new_dcm_file = new_png_file.split('.')[0]
                    new_folder = f'{patient}_{week}_{year}{hour}'
                    if patient != last_name or week != last_week:
                        png_dir_rename = os.path.join(os.path.join(des_png_path, roomdir), new_folder)
                        dcm_dir_rename = os.path.join(os.path.join(des_dcm_path, roomdir), new_folder)
...
def match(result, folder):
    ...
    try:
        patient = re.search(r'([a-zA-Z ]{2,}{[2-5]\d}?)F?', folder).group(1).strip()
...
if __name__ == '__main__':
    png_path = r'./DOING/20230623_png__'
    dcm_path = r'./DOING/20230623_dcm__'
    des_png_path = r'./DOING/20230623_png_rename__'
    des_dcm_path = r'./DOING/20230623_dcm_rename__'
